...
db.serialize(() => {
    ...
    db.run('insert into TB_PESSOAS (name) values (?)', ['Francisca Nascimento'], (error) => {
        if (error) console.log(error);
    });

    db.run('insert into TB_PESSOAS (name) values (?)', ['Narcélia Oliveira'], (error) => {
        if (error) console.log(error);
    });

    db.run('insert into TB_PESSOAS (name) values (?)', ['Carlos Haroldo'], (error) => {
        if (error) console.log(error);
    });
    ...
    db.run('UPDATE "TB_PESSOAS" SET name= "Giovana Joice" WHERE id=1', (error) => {
        if (error) console.log(error);
    });
